...
export async function getSeasonStats(
    ...
    snapshot.forEach((doc) => {
        ...
        const addPlayer = (target: Record<string, PlayerStats>, player: any) => {
        ...
        // lisa tiimid, kui neid veel pole
        if (!teamsMap[awayTeam]) teamsMap[awayTeam] = {};
        if (!teamsMap[homeTeam]) teamsMap[homeTeam] = {};

        awayPlayers.forEach((p) => addPlayer(playersMap, p));
        homePlayers.forEach((p) => addPlayer(playersMap, p));
        awayPlayers.forEach((p) => addPlayer(teamsMap[awayTeam], p));
        homePlayers.forEach((p) => addPlayer(teamsMap[homeTeam], p));
    });

    // teisenda tiimid objektidest massiivideks
